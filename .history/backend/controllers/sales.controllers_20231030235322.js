...
exports.generateOrdersPDF = async (req, res) => {
  ...
  const foundCustomer = await SalesSchema.find(customerId);

  const filteredBydate = foundCustomer.filter(
    (element) =>
  ...
  <thead>
   ...
   <tr>
    ...
    const stream = await fs.createWriteStream(`${customerName}.pdf`);
    doc.pipe(stream);
    doc.fontSize(12).addContent(htmlForPDF, { align: "center" });
    doc.end();
