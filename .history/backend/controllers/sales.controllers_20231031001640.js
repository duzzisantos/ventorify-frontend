...
exports.generateOrdersPDF = async (req, res) => {
  ...
  const foundCustomer = await SalesSchema.find({ customerId: customerId });
  ...
  <thead>
   ...
   <tr>
    ...
    const stream = fs.createWriteStream(`${customerName}.pdf`);
    doc.pipe(stream);
    doc.fontSize(12).text(htmlForPDF, { align: "center" });
    doc.end();
  } else {
