...
exports.create = async (req, res) => {
  try {
    // Find the newest data in MainWarehouse
    const newestData = await MainWarehouse.findOne(
      {},
      {},
      { sort: { createdAt: -1 } }
    );
    ...
    if (newestData) {
      ...
      const { __v, ...dataWithoutVersion } = newestData;

      // Insert or update the newest data in BackupWarehouse
      const filter = { _id: newestData._id };
      const update = { $set: dataWithoutVersion };
    ...
    res.status(500).json({
      message: "Internal server error occurred in warehouse backup",
    });
  }
};
