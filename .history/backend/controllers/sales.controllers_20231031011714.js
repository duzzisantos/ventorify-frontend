...
exports.generateOrdersPDF = async (req, res) => {
  const { customerId } = req.body;
  await SalesSchema.find(customerId).then(async (data) => {
    ...
    if (customerId) {
      ...
      res.setHeaders("Content-Disposition", 'attachment; filename="table.pdf"');
  ...
  <thead>
   ...
   <tr>
    ...
    <td>${total}</td>
      ...
      await page.pdf({ path: `${customerName}-order.pdf`, format: "A4" });
      await browser.close();
    } else {
