...
import { CartFill, Table } from "react-bootstrap-icons";
...
const ShoppingBasket = ({
  ...
  const generateOrderSummaryUsingDate = async () => {
    const bySalesDate = new Date(Date.now()).toLocaleDateString();
    try {
      const response = await axios.get(
        `http://localhost:4000/api/sales/?bySalesDate=${bySalesDate}&customerId=${customerId}`
      );
        ...
        <div className="vstack gap-2">
          ...
          <section className="my-3 p-3 shadow-sm bg-white py-3 rounded-2">
            ...
            <Button
              ...
              onClick={generateOrderSummaryUsingDate}
            >
              Generate Summary
            </Button>
            {generateOrder.map((element) => (
              <>
                <span>{element.product}</span>{" "}
                <span>{element.quantitySold}</span>
              </>
            ))}
            ...
            <Table>
              ...
              <tbody>
                {generateOrder.map((item) => (
                  <tr key={item._id}>
                    <td>{item.salesOperations.product}</td>
